...
const useRating = () => {
  const postRating = async (
    ...
    return await fetchData<MessageResponse>(
      import.meta.env.VITE_PUBLIC_MEDIA_API + '/ratings',
      options,
    );
  };
  ...
  const getRatingByMediaId = async (media_id: number) => {
    ...
    return await fetchData<{average: number}>(
      import.meta.env.VITE_PUBLIC_MEDIA_API + '/ratings/average/' + media_id,
    );
  };
  ...
  const getUserRatings = async (token: string) => {
    ...
    return await fetchData<Rating[]>(
      import.meta.env.VITE_PUBLIC_MEDIA_API + '/ratings/byuser',
      options,
    );
  };
